...
// import '../../domain/usecases/submit_schedule.dart';
// import '../../data/models/schedule_request.dart';
// import 'study_schedule_state.dart';

// class StudyScheduleCubit extends Cubit<StudyScheduleState> {
//   final FetchQuestions fetchQuestionsUseCase;
//   final SubmitSchedule submitScheduleUseCase;

//   StudyScheduleCubit(
//     this.fetchQuestionsUseCase,
//     this.submitScheduleUseCase,
//   ) : super(SSInitial());
...
//       final qs = await fetchQuestionsUseCase(token);
...
//   Future<void> sendAnswers(ScheduleRequest request) async {
//     emit(SSLoading());
//     try {
//       final prefs = await SharedPreferences.getInstance();
//       final token = prefs.getString('token') ?? '';
//       if (token.isEmpty) throw Exception('No auth token');
//       final res =
//           await submitScheduleUseCase(token: token, request: request);
